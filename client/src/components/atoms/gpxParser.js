...
const parseGpx = (file) => {
    try {
		...
		const length = Math.round(parseFloat(gpx.tracks[0].distance.total).toFixed(2));
		const ascent = Math.round(parseFloat(gpx.tracks[0].elevation.max - gpx.tracks[0].elevation.min).toFixed(2));
