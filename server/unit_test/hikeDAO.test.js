...
describe('Hike DAO unit test', () => {
    beforeAll(async () => {
        ...
        sql = "INSERT INTO Hike(title, length, expectedTime, ascent, difficulty, startPointId, endPointId, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?);";
        res = await dbManager.query(sql, ["title 1", 1000, 120, 300, mid, 1, 2, "description 1"]);
        res = await dbManager.query(sql, ["title 2", 2000, 180, 500, high, 1, 4, "description 2"]);
        res = await dbManager.query(sql, ["title 3", 1500, 100, 200, low, 3, 4, "description 3"]);
        res = await dbManager.query(sql, ["title 4", 1600, 120, 350, mid, 2, 4, "description 4"]);
    ...
    const hike1 = new Hike(1, "title 1", 1000, 120, 300, mid, 1, 2, "description 1");
    const hike2 = new Hike(2, "title 2", 2000, 180, 500, high, 1, 4, "description 2");
    const hike3 = new Hike(3, "title 3", 1500, 100, 200, low, 3, 4, "description 3");
    const hike4 = new Hike(4, "title 4", 1600, 120, 350, mid, 2, 4, "description 4");
